...
async function generateMetadata(novels) {
  ...
  function appendGlobalNovelMetadata(folder) {
    ...
    let contributors;
    try {
      contributors = yaml.safeLoad(fs.readFileSync(folder + "/contributors.yml"));
    } catch (err) {
      console.error(chalk.bold.red(`Can't find contributors.yml for novel ${novel}.`));
    }
    if (!global.contributors) global.contributors = {};
    global.contributors[novel] = contributors;
  }
}
...
async function compileChapter(folder, images, id) {
  return new Promise((resolve, reject) => {
    ...
    glob(path.resolve(folder, "contents/**/*.md"), (err, files) => {
      files.forEach((file) => {
        ...
        chapters.push(file);
        for (contribution of contribution_roles) {
          const contributor = meta.attributes[contribution];
          if (contributor && rev_share_per_chapter[contribution]) {
            contributions[contributor] =
              (contributions[contributor] || 0) + rev_share_per_chapter[contribution];
          }
        }
      });

      const rev_share = Object.keys(contributions).map((contributor) => {
        return `${contributors[novel][contributor]}#${contributions[contributor]}`;
      });
  ...
  function calculateContribution(contribution, person) {
    const pointer = "";
  }
}
